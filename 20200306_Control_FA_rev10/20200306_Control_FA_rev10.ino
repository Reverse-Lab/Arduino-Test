...
const int motor_delay = 5; // 회전속도 (작을수록 빨리회전 : Minimum 5)
//int M_motor_delay = 5;
...
void loop()
{
  ...
  slave.run(); delay(30);
  if (DO3 <= 0) teachState = LOW; //&& digitalRead(GREEN_LED) == HIGH)digitalWrite(GREEN_LED,LOW);
  if (DO3 >= 1) teachState = HIGH; //&& digitalRead(GREEN_LED) == LOW)digitalWrite(GREEN_LED,HIGH);
      ...
      while (markState == LOW)                        // 마크 검출 시작
        {
          ...
          digitalWrite(enPin, LOW);          // slave.run(); delay(30);
        ...
        digitalWrite(enPin, LOW); //slave.run(); delay(100);
        step(CW, 1);                                // 감속 스텝 적용
