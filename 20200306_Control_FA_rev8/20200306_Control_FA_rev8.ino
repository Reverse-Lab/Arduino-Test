...
void loop()
{
  ...
  { if (t_Count == 1) 
    { 
      regBank.set(10005,0);  slave.run(); delay(100);
      switch(teachoutState){ // 티칭 버튼 누른 횟수 판단
        case 0 :             // 처음 누른 경우
              ...
              digitalWrite (solpushPin, HIGH);    regBank.set(10002,0);  slave.run(); delay(100);// 푸쉬 솔 전진
        ...
        case 2 :                                            // 버튼 누른 횟수 2인 경우
              ...
              delay(1000);                                  // 1초 대기
              digitalWrite (solpushPin, LOW); regBank.set(10002,1);  slave.run(); delay(100);// 푸쉬 솔 후진
        ...
        default : break;
              //teachoutState = 0;
      }  t_Count =0; regBank.set(10005,1); slave.run(); delay(100);
    } 
  };
  
  if (startState == LOW ) // 근접센서 LOW는 대기위치로 인식
  {
    digitalWrite(enPin, HIGH); regBank.set(10003,1);  slave.run(); delay(100);// 모터 기동준비 해재 (EN 신호가 HIGH면 모터드라이버는 기동준비 안된걸로 판단!!)
    markState = digitalRead (markPin);
    ledState = digitalRead (teachLED); 
    if (ledState == 0 ) 
    {
     digitalWrite(solpushPin, LOW); regBank.set(10002,1);  slave.run(); delay(100);// 푸쉬 솔 작동 (LOW : 후진)
    }  
  } 
  else // 근접센서 LOW인 경우 (작업 개시로 판단)
  {
      digitalWrite(solpushPin, HIGH); regBank.set(10002,0); slave.run();// 푸쉬 솔 작동 (HIGH : 전진) 
      delay (100);                                          // 푸쉬 솔 전진 완료 대기 (0.1초)
      
      while (markState == LOW)        // 마크 검출 시작
        {
          regBank.set(10004,1);  slave.run(); delay(100);
        ...
        regBank.set(10004,0); slave.run();// delay(100);
        digitalWrite(enPin, LOW); regBank.set(10003,0);  slave.run(); delay(100);
        step(CW, 1);                                // 감속 스텝 적용
        delayMicroseconds(100);
        digitalWrite(enPin, HIGH); regBank.set(10003,1); slave.run(); delay(100);
