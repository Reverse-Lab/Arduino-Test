  ...
  { if (t_Count == 1) 
    {
      switch(teachoutState){ // 티칭 버튼 누른 횟수 판단
        ...
        case 2 :                                   // 버튼 누른 횟수 2인 경우
              digitalWrite (teachoutPin, LOW);     // 센서로 출력 LOW
              delay(1000);                         // 1초 대기
              digitalWrite (solpushPin, LOW);      // 푸쉬 솔 후진
              digitalWrite (backlightPin, LOW);    // 백 라이트 OFF
              digitalWrite (teachLED, LOW);
              teachoutState = 0;                   // 리모트 티칭 완료, 버튼 누른횟수 초기화
              //Serial.println(teachoutState);
        default : break;
              //teachoutState = 0;
      }  t_Count =0;
