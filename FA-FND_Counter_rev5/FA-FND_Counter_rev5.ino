 ...
 */ 
 ...
 pinMode (buzzerPin, OUTPUT); // 02-28 추가
...
void loop() {
  ...
  if (sensorState == LOW)
  {
    ...
    //if ((setCount - buzzerCount) <= value) setState = 0; // 02-28 추가
  ...
  // 02-25 추가
  if (solState == 1)// 카운터 릴레리 동작 ON
  {
    if (solcheckState == 0) //솔레노이드 동작 OFF
    {
      digitalWrite (solPin, HIGH); // 솔레노이드 동작 ON
      delay(10);
      solcheckState = 1; // 솔레노이드 동작체크 ON
    } 
    else // 솔레노이드 동작 ON
    {
      digitalWrite (solPin, LOW); //솔레노이드 동작 OFF
      delay(10);
      solcheckState = 0; // 솔레노이드 동작체크 OFF
    }
  }
  solState = 0;

  // 02-28 추가
  if ((setCount - buzzerCount) <= value) 
  {
